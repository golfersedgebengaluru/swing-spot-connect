...
async function reverseRevenueAndInvoice(adminClient: any, bookingId: string) {
  try {
    ...
    const { data: cnData } = await adminClient.rpc("get_next_invoice_number", {
    ...
    const cnNumber = (typeof cnData === "string" ? cnData : (cnData as any)) as string | undefined;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "guest_booking") {
      ...
      if (!isBackdated && guest_email) {
        const addToCalendarUrl = await generateAddToCalendarUrl(adminClient, booking.id, {
          ...
          summary: `Bay Booking — ${bay_name || city}`,
          description: `Your booking at ${bay_name || city} is confirmed.`,
          location: `${bay_name || city}, ${city}`,
        });
        try {
          await fetch(`${Deno.env.get("SUPABASE_URL")}/functions/v1/send-notification-email`, {
            method: "POST",
    ...
    if (action === "approve_booking") {
      ...
      const durationHoursApprove = booking.duration_minutes / 60;
      const hoursNeeded = booking.session_type === "coaching" ? coachingHours * durationHoursApprove : durationHoursApprove;
