...
BEGIN
  ...
  -- Highest trailing number already used for this prefix (regardless of which
  -- GSTIN was stamped on the invoice — historic rows are inconsistent) so the
  -- generated number can never collide.
  SELECT COALESCE(MAX(
    NULLIF(regexp_replace(invoice_number, '^.*/([0-9]+)$', '\1'), '')::int
  ), 0)
  INTO v_max_existing
  FROM invoices
  WHERE invoice_number LIKE (p_prefix || '/%')
  ...
  LOOP
    ...
    -- Collision: jump past the highest number currently in use for this prefix
    -- instead of stepping one at a time.
    SELECT GREATEST(
      v_next,
      COALESCE(MAX(NULLIF(regexp_replace(invoice_number, '^.*/([0-9]+)$', '\1'), '')::int), 0)
    )
    INTO v_max_existing
    FROM invoices
    WHERE invoice_number LIKE (p_prefix || '/%')
