...
BEGIN
  ...
  -- Registration is explicit (is_gst_registered) AND requires a real GSTIN.
  ...
  -- 1) the stamped product_id column (set by resolve_revenue_product trigger or
  --    passed explicitly by the shared revenue-ledger writer)
  v_product_id := v_rev.product_id;

  -- 2) legacy: explicit product_id in metadata (older purchases / shop orders)
  ...
  -- 3) booking → bay_pricing.service_product_id (by city + session_type)
  ...
  -- Unregistered city: plain bill of supply, no tax component at all.
