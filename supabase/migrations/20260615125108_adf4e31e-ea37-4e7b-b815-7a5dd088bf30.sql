-- ─────────────────────────────────────────────────────────────
-- Hotfix A: harden auto_create_invoice_for_revenue + block product
-- deletion when the product is referenced by issued invoices.
-- ─────────────────────────────────────────────────────────────

-- (1) Replace auto_create_invoice_for_revenue so that a missing/deleted
--     product no longer crashes the function via an uninitialised RECORD.
--     We now track lookup state explicitly with a boolean and emit a clear
--     WARNING when the metadata product_id can't be resolved.
...
BEGIN
  ...
  IF v_product_id IS NOT NULL THEN
    ...
    IF FOUND THEN
      v_has_product := true;
    ELSE
      -- Product was deleted between booking and revenue capture.
...
-- (2) Guard: block deletion of products that are referenced from issued
--     invoice line items. Cancelled invoices are fine to lose the product
--     pointer (they are reversed anyway). bay_pricing is already nulled by
--     the admin UI before deletion.
CREATE OR REPLACE FUNCTION public.prevent_referenced_product_delete()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_invoice_ref_count integer;
BEGIN
  SELECT COUNT(*) INTO v_invoice_ref_count
    FROM public.invoice_line_items li
    JOIN public.invoices i ON i.id = li.invoice_id
   WHERE li.product_id = OLD.id
     AND i.status = 'issued';

  IF v_invoice_ref_count > 0 THEN
    RAISE EXCEPTION 'Cannot delete product "%": it is referenced by % issued invoice line item(s). Mark it inactive (in_stock = false / bookable = false) instead.',
      OLD.name, v_invoice_ref_count
...
DROP TRIGGER IF EXISTS trg_prevent_referenced_product_delete ON public.products;
CREATE TRIGGER trg_prevent_referenced_product_delete
  BEFORE DELETE ON public.products
  FOR EACH ROW EXECUTE FUNCTION public.prevent_referenced_product_delete();