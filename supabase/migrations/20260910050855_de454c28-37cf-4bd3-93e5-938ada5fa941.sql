...
BEGIN
  ...
  -- member's profile before giving up, then fail loudly at the point of the
  -- mistake instead of surfacing months later in a report.
  ...
  IF v_city IS NULL AND p_user_id IS NOT NULL THEN
    SELECT NULLIF(btrim(COALESCE(pr.city, '')), '')
      INTO v_city
...
-- Last line of defence: even a future bug that bypasses record_revenue cannot
-- land a paid row with no city.
...
DROP TRIGGER IF EXISTS validate_revenue_city_before_insert ON public.revenue_transactions;
CREATE TRIGGER validate_revenue_city_before_insert
BEFORE INSERT ON public.revenue_transactions
FOR EACH ROW EXECUTE FUNCTION public.validate_revenue_city_before_insert();