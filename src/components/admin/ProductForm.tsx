...
// SKUs are generated by the database on save (see products_manage_sku trigger).
// The client never invents one — it only previews / displays.
...
export function ProductForm({ product, onSave, onCancel }: ProductFormProps) {
  ...
  const handleSave = async () => {
    ...
    const payload: any = {
      ...
      // Existing SKUs are immutable; sending null asks the database to (re)generate one.
      sku: regenerateSku ? null : (product?.id ? form.sku || null : null),
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <Label>SKU</Label>
          <Input value={product?.id ? form.sku : ""} readOnly disabled placeholder={`${skuPreview}-XXX (auto)`} className="font-mono text-xs" />
          {product?.id ? (
            <div className="flex items-center gap-2 mt-1">
              <Button type="button" variant="ghost" size="sm" className="h-6 px-2 text-[11px]"
                onClick={() => setRegenerateSku((v) => !v)}>
                {regenerateSku ? "Keep current SKU" : "Regenerate SKU"}
              </Button>
              {regenerateSku && (
                <span className="text-[11px] text-muted-foreground">New code assigned on save.</span>
              )}
            </div>
          ) : (
            <p className="text-[11px] text-muted-foreground mt-1">Generated automatically when you save.</p>
          )}
        </div>
        <div>
