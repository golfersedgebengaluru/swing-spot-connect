...
export async function leagueServiceInvoke(path: string, method: string, body?: any) {
  return invoke(path, method, body);
}
...
export type LandingLeague = Pick<League, "id" | "name" | "venue_id" | "status" | "allowed_team_sizes" | "show_on_landing" | "landing_note" | "price_per_person" | "currency" | "gst_mode" | "gst_rate" | "sac_code">;
  ...
  return useQuery<LandingLeague[]>({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("leagues")
        .select("id, name, venue_id, status, allowed_team_sizes, show_on_landing, landing_note, price_per_person, currency, gst_mode, gst_rate, sac_code")
...
/** All locations across every city of the league (id, name, software). */
export function useLeagueAllLocations(leagueId: string | null) {
  return useQuery<Array<{ id: string; name: string; software?: string; city_id: string }>>({
    queryKey: ["league-all-locations", leagueId],
    queryFn: async () => {
      const cities: { id: string }[] = await invoke(`/leagues/${leagueId}/cities`, "GET");
