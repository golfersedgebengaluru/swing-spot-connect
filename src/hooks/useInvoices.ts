...
export function useGstProfile(city?: string) {
  return useQuery({
    ...
    enabled: !!city,
    queryFn: async () => {
      const { data, error } = await supabase.from("gst_profiles")
        .select("*")
        .eq("city", city)
        .maybeSingle();
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (invoice.invoice_category === "booking" && invoice.revenue_transaction_id) {
        const { data: revTxn } = await supabase.from("revenue_transactions")
          .select("booking_id")
  ...
  return useMutation({
    mutationFn: async (params: CreateInvoiceParams) => {
      ...
      if (shouldCreateProfile) {
        ...
        if (!existingProfile) {
          const { data: newProfile, error: profErr } = await supabase.from("profiles")
            .insert({
